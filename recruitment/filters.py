...
class CandidateFilter(FilterSet):
    """
    Filter set class for Candidate model - recruitment-agnostic candidate profiles
    ...
    # Date filters for candidate profile
    date_created_from = django_filters.DateFilter(
        field_name="created_at",
        lookup_expr="gte",
        widget=forms.DateInput(attrs={"type": "date"}),
    )
    date_created_till = django_filters.DateFilter(
        field_name="created_at",
        lookup_expr="lte",
        widget=forms.DateInput(attrs={"type": "date"}),
    )
    ...
    def _update_field_labels(self, form_fields):
        ...
        field_label_map = {
            ...
            "interview_date_from": f"{interview_date_label} From",
            "interview_date_till": f"{interview_date_label} Till",
...
class StageFilter(FilterSet):
    ...
    def filter_by_name(self, queryset, _, value):
        ...
        # Filter by stage managers
        stage_manager_query = queryset
        if first_name and last_name:
            stage_manager_query = queryset.filter(
        ...
        # Filter by stage interviewers
        stage_interviewer_query = queryset
        if first_name and last_name:
            stage_interviewer_query = queryset.filter(
                stage_interviewers__employee_first_name__icontains=first_name,
                stage_interviewers__employee_last_name__icontains=last_name,
            )
        elif first_name:
            stage_interviewer_query = queryset.filter(
                stage_interviewers__employee_first_name__icontains=first_name
        ...
        queryset = queryset | stage_queryset | recruitment_query | stage_manager_query | stage_interviewer_query
        return queryset
...
class CandidateApplicationFilter(FilterSet):
    ...
    candidate_application = django_filters.ModelMultipleChoiceFilter(
    ...
    scheduled_from = django_filters.DateFilter(
        field_name="candidate_application_interview__interview_date",
        lookup_expr="gte",
        widget=forms.DateInput(attrs={"type": "date"}),
    )
    probation_end = django_filters.DateFilter(
    ...
    def pipeline_search(self, queryset, _, value):
        """
        This method is used to include the candidate applications when they in the recruitment/stages
        """
        queryset = (
    ...
    def start_onboard_method(self, queryset, _, value):
        """
        This method will include the candidate applications whether they are on the onboarding pipeline stage
        """
        # For CandidateApplication, we check if they have an onboarding stage
        return queryset.filter(start_onboard=True)
    ...
    def filter_mail_sent(self, queryset, name, value):
        # For CandidateApplication, we can implement this differently
        return queryset.filter(start_onboard=(value))

    def filter_joining_set(self, queryset, name, value):
        return queryset.filter(joining_date__isnull=(not value))


class InterviewScheduleApplicationFilter(FilterSet):
    """
    Filter set class for InterviewScheduleApplication model
    """

    candidate_name = django_filters.CharFilter(
        field_name="candidate_application_id__name", lookup_expr="icontains"
    )
    interview_date = django_filters.DateFilter(
        field_name="interview_date",
        widget=forms.DateInput(attrs={"type": "date"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in self.form.fields:
            self.form.fields[field].widget.attrs["id"] = str(uuid.uuid4())


class StageNoteApplicationFilter(FilterSet):
    """
    Filter set class for StageNoteApplication model
    """
